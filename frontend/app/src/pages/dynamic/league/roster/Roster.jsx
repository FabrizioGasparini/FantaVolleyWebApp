...
const Roster = () => {
    ...
    const getRoster = async () => {
        try {
            ...
            const response = await axios.post('/api/v1/rosters/read', { 'league_code': invite_code }, {
                headers: {
                ...
                try {
                    ...
                    response.data.players.forEach(player => {
                        roster.players.forEach(rosterPlayer => {
                            if (player.id == rosterPlayer.player_id && rosterPlayer.court_position == -1)
                            {
                                output.push(player)
                            }
                        })
            ...
            <h1 className='header'>Formazione</h1>
            {
                ...
                (<div className="court">
                    ...
                    <div className="panchina">
                        ...
                        <CourtPosition role="Palleggiatore"
                            court_position={9}
                            league_code={invite_code}
                            player={getPlayer(9)}
                            onDrop={() => onDropCard()}
                        ...
                        <CourtPosition role="Schiacciatore"
                            court_position={10}
                            league_code={invite_code} 
                            player={getPlayer(10)} 
                            onDrop={() => onDropCard()}
                    ...
                    <ul>
                        {
                            unusedPlayers.map((player, index) => (
                                <PlayerCard key={index} player={player} small draggable court_position={-1} />
                            ))
                        }
                    </ul>
                </div> : ''
